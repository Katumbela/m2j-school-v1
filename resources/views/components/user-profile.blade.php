    ...
    <h3>{{$user->name}} <span class="label label-danger">{{ucfirst($user->role)}}</span> <span class="label label-primary">{{ucfirst($user->gender)}}</span>
      ...
      <script>
        document.getElementById('btnPrint').addEventListener('click', function() {
          var printContents = document.getElementById('profile-content').innerHTML;
          ...
          // Reattach event listeners after restoring the page
          document.getElementById('btnPrint').addEventListener('click', arguments.callee);
        });
      </script>
      @endif
    </h3>
